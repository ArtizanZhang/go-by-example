...
func workman(id int, jobs <-chan int, results chan<- int) {
	for j := range jobs {
...
func main() {
	...
	// This also ensures that the worker goroutines have finished. An alternative way to wait for multiple goroutines is to use a WaitGroup.
	for a := 1; a <= numJobs; a++ {
		<-results
		//r := <-results
